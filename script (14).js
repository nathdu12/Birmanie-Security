...
window.pwCheck = function(){
  ...
  if(val === 'BIrm@niEGl0rySecure#2026'){
    ...
    if(gate){ gate.style.transition='opacity .5s'; gate.style.opacity='0'; setTimeout(function(){ gate.style.display='none'; }, 500); }
    const lk = document.getElementById('init-lock');
    if(lk) lk.remove();
    document.querySelectorAll('.hdr,.nav,.main').forEach(function(el){ el.style.display=''; });
    setTimeout(function(){
      const ldr = document.getElementById('ldr');
      if(ldr){ ldr.classList.add('out'); setTimeout(function(){ ldr.style.display='none'; showToast('ACCÈS AUTORISÉ',2000); }, 900); }
    }, 400);
  } else {
    if(err){ err.textContent='CODE INVALIDE — ACCÈS REFUSÉ'; err.style.opacity='1'; }
    inp.value='';
    inp.style.borderColor='rgba(255,24,64,.5)';
    setTimeout(function(){ if(err) err.style.opacity='0'; inp.style.borderColor=''; }, 3000);
  }
};

// Si déjà connecté au chargement de la page
...
let _dashPeriod=3,_dashChart=null;
let _pcCache={};
...
function enterSite(){
  ...
  // Supprimer init-lock et déverrouiller le contenu
...
async function api(p,opts={}){
  // Backend non disponible - toutes les données passent par Dynmap/API publique NG
  throw new Error('Endpoint non disponible');
}

async function apiP(p,b){
...
function _connectSSE(){
  // SSE désactivé — backend non disponible
}
...
async function init(){
  ...
  rHist();let ok=false;
  try{
    const r=await fetch('https://publicapi.nationsglory.fr/online/lime',{headers:{'x-api-key':NG_API_KEY},signal:AbortSignal.timeout(4000)});
    ok=r.ok;
  }catch(e){ok=false;}$('scan-led').className=ok?'led on':'led off';
  if(ok){
...
async function loadDash(){
  const ov=document.getElementById('srv-overview');
  const wq=document.getElementById('wl-quick');
  if(ov&&!ov.children.length)ov.innerHTML=ld();
  try{
    const all=await dynmapGetAll();
    const srvList=Object.keys(DYNMAP_BASES);
    let tot=0;srvList.forEach(s=>tot+=(all[s]||[]).length);
    if(typeof animStat==='function')animStat('st-total',tot);
    const el=document.getElementById('st-total');if(el)el.textContent=tot;
    if(ov){
      ov.innerHTML=srvList.map(s=>{
        const pl=(all[s]||[]);const cnt=pl.length;
        const mx=Math.max(...srvList.map(ss=>(all[ss]||[]).length),1);
        const pct=Math.round(cnt/mx*100);
        return `<div class="so" onclick="typeof fOL!=='undefined'&&fOL('${s}')">
          ...
          <div class="so-pl">${pl.slice(0,8).map(p=>`<span class="tag" onclick="event.stopPropagation();typeof qCA!=='undefined'&&qCA('${p.name}')">${p.name}</span>`).join('')}${cnt>8?`<span style="font-family:var(--M);font-size:.44rem;color:var(--t3)">+${cnt-8}</span>`:''}</div>
        </div>`;
      }).join('');
    }
    ...
    if(wq){
      const s='lime';
      const pl=all[s]||[];
      const lp=pl.map(p=>p.name.toLowerCase());
      const on=WL.filter(p=>lp.includes(p.toLowerCase()));
      const off=WL.filter(p=>!lp.includes(p.toLowerCase()));
      ...
      ].join(''):'<div class="empty">Watchlist vide</div>';
      const wonline=document.getElementById('st-wonline');if(wonline)wonline.textContent=on.length;
      const wcount=document.getElementById('st-wcount');if(wcount)wcount.textContent=WL.length;
      updateWeather(on.length);
    }
    ...
    if(ov)ov.innerHTML=`<div class="empty" style="color:var(--red)">Erreur : ${e.message}</div>`;
  }
}
