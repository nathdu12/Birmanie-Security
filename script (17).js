...
async function doCheck(){
  ...
  res.innerHTML=ld();
  try{
    // 1. Récupérer infos du pays (power, claims, membres) via API NG
    let countryInfo=null,membersList=[];
    try{
      const r=await fetch(`${NG_API_BASE}/country/${s}/${encodeURIComponent(c)}`,{headers:{'x-api-key':NG_API_KEY},signal:AbortSignal.timeout(6000)});
      if(r.ok){
        const d=await r.json();
        countryInfo=d;
    ...
    // 2. Si pas de membres via API, fallback membres dynmap
    // 3. Récupérer joueurs en ligne sur ce serveur via dynmap
