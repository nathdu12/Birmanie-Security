...
async function doCheck(){
  ...
  if(!s||!raw)return;
  // Charger pays : API NG + fallback liste statique
  ...
  res.innerHTML=ld();
  try{
    // 1. Récupérer infos du pays (power, claims, membres) via API NG + fallback CORS proxy
    let countryInfo=null,membersList=[];
    ...
    for(const ep of countryEndpoints){
      try{
        const r=await fetch(ep,{headers:{'x-api-key':NG_API_KEY},signal:AbortSignal.timeout(6000)});
        if(r.ok){
          const d=await r.json();
          countryInfo=d;
    ...
    // 2. Récupérer joueurs en ligne sur ce serveur via dynmap
    const onlinePlayers=await dynmapGetPlayers(s);
    const onlineNames=onlinePlayers.map(p=>p.name.toLowerCase());

    // 4. Croiser membres du pays avec joueurs en ligne
    let onlineMembers=[];
    let noMemberData=false;
    if(membersList.length){
      onlineMembers=membersList.filter(m=>onlineNames.includes(m.toLowerCase()));
    } else {
      // Pas de liste de membres disponible : impossible de filtrer par pays
      noMemberData=true;
    }

    // 5. Construire le bloc infos pays
    ...
    const totalMembers=membersList.length||countryInfo?.members_count||countryInfo?.members||'?';
    ...
    const infoBloc=(power||claims||mmr)?`<div style="font-family:var(--M);font-size:.49rem;color:var(--t3);margin:.35rem 0 .05rem;display:flex;gap:.8rem;flex-wrap:wrap;align-items:center">
      ${claims?`<span>🏴 <b style="color:var(--t1)">${claims}</b> claims</span>`:''}
      ${power?`<span>⚡ <b style="color:${isSP?'var(--red)':'var(--grn)'}">${power}</b>/<b style="color:var(--t2)">${maxpower}</b> power${isSP?` <span style="color:var(--red);font-size:.44rem">▼ SOUS-POWER (${claims-power})</span>`:''}</span>`:''}
      ${mmr?`<span>🏆 <b style="color:var(--t1)">${mmr}</b> MMR</span>`:''}
      ${leader?`<span style="display:inline-flex;align-items:center;gap:.3rem"><img src="https://skins.nationsglory.fr/face/${leader}/32" style="width:20px;height:20px;border-radius:3px;border:1px solid var(--b2);image-rendering:pixelated;flex-shrink:0" onerror="this.style.display='none'">👑 <b style="color:var(--t1)">${leader}</b></span>`:''}
    </div>${powerBar}`:'';

    const dynmapLink=`https://${s}.nationsglory.fr/`;
    if(noMemberData){
      // Pas de liste membres : on indique que le scan Dynmap a fonctionné mais les données pays sont indisponibles
      res.innerHTML=`<div class="res ok" style="border-color:var(--org)">
        <div class="rt">Pays : <a href="${dynmapLink}" target="_blank" rel="noopener" style="color:var(--blue-pale);text-decoration:none">${c} ↗</a></div>
        ${infoBloc}
        <div style="margin-top:.5rem;color:var(--org);font-family:var(--M);font-size:.6rem">⚠ Liste des membres indisponible (API pays inaccessible)</div>
        <div style="font-family:var(--M);font-size:.5rem;color:var(--t3);margin-top:.3rem">
          ${onlinePlayers.length} joueurs connectés sur ${s.toUpperCase()} en ce moment — impossible de filtrer par pays sans la liste des membres.
        </div>
        <div style="margin-top:.5rem;display:flex;flex-wrap:wrap;gap:.3rem">
          ${onlinePlayers.slice(0,30).map(p=>`<span class="tag" style="font-family:var(--M);font-size:.5rem">${p.name}</span>`).join('')}
          ${onlinePlayers.length>30?`<span style="font-family:var(--M);font-size:.48rem;color:var(--t3)">+${onlinePlayers.length-30} autres</span>`:''}
        </div>
      </div>`;
    } else if(onlineMembers.length===0){
      res.innerHTML=`<div class="res ok">
        <div class="rt">Pays : <a href="${dynmapLink}" target="_blank" rel="noopener" style="color:var(--blue-pale);text-decoration:none">${c} ↗</a> — ${typeof totalMembers==='number'?totalMembers+'&nbsp;membres':'?'}</div>
        ${infoBloc}
        <div style="margin-top:.5rem;color:var(--grn);font-family:var(--M);font-size:.65rem">✓ Aucun membre connecté sur ${s.toUpperCase()}</div>
        <div style="font-family:var(--M);font-size:.5rem;color:var(--t3);margin-top:.4rem">Source : Dynmap ${s.toUpperCase()} — ${onlinePlayers.length} joueurs scannés</div>
      </div>`;
    }else{
      res.innerHTML=`<div class="res err">
        <div class="rt">Pays : <a href="${dynmapLink}" target="_blank" rel="noopener" style="color:var(--blue-pale);text-decoration:none">${c} ↗</a> — <b style="color:var(--red)">${onlineMembers.length}</b> membre${onlineMembers.length>1?'s':''} connecté${onlineMembers.length>1?'s':''}${typeof totalMembers==='number'?' / '+totalMembers:''}</div>
        ${infoBloc}
...
const ACT_LS_KEY='mg_act_history_v1';
const ACT_MAX_POINTS=288; // 24h à 5min d'intervalle
...
function _actStartAutoSnapshot(){
  ...
  },5*60*1000); // snapshot toutes les 5 minutes
}
...
async function loadActivity(){
  ...
  try{
    // Prendre un snapshot immédiat puis lire l'historique
    const allPts=await _actSnapshot();
    _actStartAutoSnapshot();
    const pts=_actFilterByHours(allPts,actPeriod);
    if(!pts.length){
      if(loadEl)loadEl.style.display='none';
      if(emptyEl){emptyEl.style.display='flex';emptyEl.textContent='Pas encore assez de données — le graphique se remplit automatiquement au fil du temps.';}
      return;
    }
    actData={points:pts};
    if(loadEl)loadEl.style.display='none';
