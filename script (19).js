...
let _dashChart=null,_dashPeriod=24;
function sndNav(){
  if(!snd)return;
...
async function doCheck(){
  const s=$('ck-srv').value,raw=$('ck-country').value.trim();
  if(!s||!raw)return;
  await getCountries(s);
  if(!cc[s]||!cc[s].length)cc[s]=STATIC_COUNTRIES_FALLBACK;
  const c=rP(raw,[...(cc[s]||[]),...STATIC_COUNTRIES_FALLBACK]);
  $('ck-country').value=c;
  const res=$('ck-result');
  res.innerHTML=ld();
  try{
    // Joueurs en ligne via Dynmap (accessible depuis le navigateur)
    const onlinePlayers=await dynmapGetPlayers(s);
    const onlineNames=onlinePlayers.map(p=>p.name.toLowerCase());

    // Membres du pays via API NG (direct + proxy CORS)
    let membersList=[],countryInfo=null;
    const eps=[
      `${NG_API_BASE}/country/${s}/${encodeURIComponent(c)}`,
      `${CORS_PROXY}${encodeURIComponent(NG_API_BASE+'/country/'+s+'/'+encodeURIComponent(c))}`
    ];
    for(const ep of eps){
      try{
        const r=await fetch(ep,{headers:{'x-api-key':NG_API_KEY},signal:AbortSignal.timeout(7000)});
        if(r.ok){
          const d=await r.json();
          countryInfo=d;
          if(d.members&&Array.isArray(d.members))membersList=d.members.map(m=>typeof m==='string'?m:(m.name||m.pseudo||'')).filter(Boolean);
          else if(d.players&&Array.isArray(d.players))membersList=d.players.map(m=>typeof m==='string'?m:(m.name||m.pseudo||'')).filter(Boolean);
          if(membersList.length)break;
        }
      }catch(e2){}
    }

    // Fallback : Claude API comme proxy pour bypasser CORS
      ...
      try{
        const clResp=await fetch('https://api.anthropic.com/v1/messages',{
          ...
          body:JSON.stringify({
            ...
            messages:[{role:'user',content:`Use web_search to fetch this URL and return ONLY a JSON array of player names who are members of the country "${c}" on the NationsGlory server "${s}". URL: ${NG_API_BASE}/country/${s}/${encodeURIComponent(c)}. Return ONLY a JSON array like ["player1","player2"] with no explanation.`}]
          })
        });
        if(clResp.ok){
          const clData=await clResp.json();
          const txt=(clData.content||[]).map(b=>b.text||'').join('').trim();
          const m=txt.match(/\[[\s\S]*?\]/);
          if(m)membersList=JSON.parse(m[0]).filter(x=>typeof x==='string');
        }
      }catch(e3){}
    }

    const power=countryInfo?.power??null;
    const maxpower=countryInfo?.maxpower??countryInfo?.max_power??null;
    const claims=countryInfo?.claims??null;
    const mmr=countryInfo?.mmr??null;
    const leader=countryInfo?.leader??countryInfo?.chief??null;
    const totalMembers=membersList.length||countryInfo?.members_count||null;
    const isSP=power!==null&&claims!==null&&power<claims;
    const powerPct=power&&maxpower?Math.min(100,Math.round(power/maxpower*100)):0;
    const powerBar=maxpower?`<div style="margin:.3rem 0 .1rem;background:var(--bg2);border-radius:3px;height:4px;overflow:hidden"><div style="height:100%;width:${powerPct}%;background:${isSP?'var(--red)':'var(--grn)'};transition:width .4s"></div></div>`:'';
    const infoBloc=(power||claims||mmr)?`<div style="font-family:var(--M);font-size:.49rem;color:var(--t3);margin:.35rem 0 .05rem;display:flex;gap:.8rem;flex-wrap:wrap;align-items:center">${claims?`<span>🏴 <b style="color:var(--t1)">${claims}</b> claims</span>`:''}${power?`<span>⚡ <b style="color:${isSP?'var(--red)':'var(--grn)'}">${power}</b>/<b style="color:var(--t2)">${maxpower}</b> power</span>`:''} ${mmr?`<span>🏆 <b style="color:var(--t1)">${mmr}</b> MMR</span>`:''} ${leader?`<span>👑 <b style="color:var(--t1)">${leader}</b></span>`:''}</div>${powerBar}`:'';
    const dynmapLink=`https://${s}.nationsglory.fr/`;

    if(!membersList.length){
      // Pas de liste membres : afficher tous les joueurs en ligne avec avertissement
      res.innerHTML=`<div class="res" style="border-color:var(--org)">
        <div class="rt" style="margin-bottom:.4rem">Pays : <a href="${dynmapLink}" target="_blank" rel="noopener" style="color:var(--blue-pale);text-decoration:none">${c} ↗</a>
          <span style="font-family:var(--M);font-size:.5rem;color:var(--org);margin-left:.8rem">⚠ Liste membres indisponible</span>
        </div>
        ${infoBloc}
        <div style="font-family:var(--M);font-size:.54rem;color:var(--t3);margin:.45rem 0 .3rem">${onlinePlayers.length} joueur${onlinePlayers.length>1?'s':''} connecté${onlinePlayers.length>1?'s':''} sur <b>${s.toUpperCase()}</b> :</div>
        <div style="display:flex;flex-wrap:wrap;gap:.3rem">
          ${onlinePlayers.slice(0,40).map(p=>`<span class="tag" style="cursor:pointer" onclick="openPlayerPanel&&openPlayerPanel('${p.name.replace(/'/g,"\\'")}')"><img src="https://skins.nationsglory.fr/face/${encodeURIComponent(p.name)}/16" style="width:14px;height:14px;border-radius:2px;image-rendering:pixelated;vertical-align:middle;margin-right:.2rem" onerror="this.style.display='none'">${p.name}</span>`).join('')}
          ${onlinePlayers.length>40?`<span style="font-family:var(--M);font-size:.48rem;color:var(--t3)">+${onlinePlayers.length-40} autres</span>`:''}
        </div>
      </div>`;
    } else {
      const onlineMembers=membersList.filter(m=>onlineNames.includes(m.toLowerCase()));
      if(onlineMembers.length===0){
        res.innerHTML=`<div class="res ok">
          <div class="rt">Pays : <a href="${dynmapLink}" target="_blank" rel="noopener" style="color:var(--blue-pale);text-decoration:none">${c} ↗</a>${totalMembers?` — ${totalMembers} membres`:''}</div>
          ${infoBloc}
          <div style="margin-top:.5rem;color:var(--grn);font-family:var(--M);font-size:.65rem">✓ Aucun membre connecté sur ${s.toUpperCase()}</div>
          <div style="font-family:var(--M);font-size:.5rem;color:var(--t3);margin-top:.4rem">Dynmap : ${onlinePlayers.length} joueurs scannés · ${membersList.length} membres dans le pays</div>
        </div>`;
      } else {
        res.innerHTML=`<div class="res err">
          <div class="rt">Pays : <a href="${dynmapLink}" target="_blank" rel="noopener" style="color:var(--blue-pale);text-decoration:none">${c} ↗</a> — <b style="color:var(--red)">${onlineMembers.length}</b>${totalMembers?' / '+totalMembers:''} membres connectés</div>
          ${infoBloc}
          <div style="margin-top:.5rem;display:flex;flex-direction:column;gap:.2rem">
            ${onlineMembers.map(m=>`<div style="display:flex;align-items:center;gap:.5rem;padding:.25rem 0;border-bottom:1px solid var(--b1)">
              <img src="https://skins.nationsglory.fr/face/${encodeURIComponent(m)}/32" style="width:24px;height:24px;border-radius:3px;border:1px solid var(--b2);image-rendering:pixelated;flex-shrink:0" onerror="this.style.display='none'">
              <span style="font-family:var(--M);font-size:.62rem;color:var(--grn)">${m}</span>
              <span style="margin-left:auto;font-size:.5rem;color:var(--t3)">${EMO[s]||''} ${s.toUpperCase()}</span>
            </div>`).join('')}
          </div>
          <div style="font-family:var(--M);font-size:.5rem;color:var(--t3);margin-top:.4rem">Dynmap : ${onlinePlayers.length} joueurs scannés · ${membersList.length} membres dans le pays</div>
        </div>`;
  ...
  }catch(e){res.innerHTML=`<div class="res err"><div class="rt">Erreur</div>${e.message}</div>`;}
}
...
async function loadDashActivityChart(){
  ...
  if(!canvas)return;
  try{
    // Montrer un message pendant le chargement Dynmap (peut prendre 5-10s)
    const em=$('dash-act-empty');
    if(em){em.style.display='flex';em.textContent='Scan Dynmap en cours...';}
    canvas.style.display='none';
    const allPts=await _actSnapshot();
    _actStartAutoSnapshot();
    const pts=_actFilterByHours(allPts,_dashPeriod);
    if(!pts.length){
      if(em){em.style.display='flex';em.textContent='Pas encore assez de données — le graphique se remplit au fil du temps (toutes les 5 min)';}
      return;
    }
...
async function cwRefreshOne(idx){
  const w=cwWatches[idx];if(!w)return;
  try{
    // Récupère la liste des membres du pays via API NG
    let membersList=[];
    try{
      const r=await fetch(`${NG_API_BASE}/country/${w.server}/${encodeURIComponent(w.country)}`,{headers:{'x-api-key':NG_API_KEY},signal:AbortSignal.timeout(6000)});
      if(r.ok){const d=await r.json();
        if(d.members&&Array.isArray(d.members))membersList=d.members.map(m=>typeof m==='string'?m:(m.name||m.pseudo||''));
        else if(d.players&&Array.isArray(d.players))membersList=d.players.map(m=>typeof m==='string'?m:(m.name||m.pseudo||''));
        w.leader=d.leader||d.chief||'';
      }
    }catch{}
    // Croise avec dynmap
    const onlinePlayers=await dynmapGetPlayers(w.server);
    const onlineNames=onlinePlayers.map(p=>p.name.toLowerCase());
    const members=membersList.filter(m=>onlineNames.includes(m.toLowerCase()));
...
async function init(){
  ...
  rHist();
  // Vérification API en parallèle — ne bloque plus le chargement
  chkAPI().then(ok=>{ const led=$('scan-led');if(led)led.className=ok?'led on':'led off'; });
  // Chargement immédiat sans attendre chkAPI
  await Promise.all([loadWL(),loadWLM()]);
  loadKP();
  await loadDash();
  startCountdown(5);
  loadDashActivityChart();
  setInterval(tickCountdown,1000);
  // Polling toutes les 30s (au lieu de 5s) pour ne pas saturer les Dynmaps
  setInterval(async()=>{ await loadWL(); await loadDash(); }, 30000);
  _connectSSE();
}
init();
...
// ── Historique activité stocké en localStorage ──────────────────────
const ACT_LS_KEY='mg_act_history_v2'; // v2 = nouvelle clé = historique repart propre
const ACT_MAX_POINTS=1440; // 24h à 1min d'intervalle
let _actSnapshotTimer=null;

function _actLoadHistory(){
  try{const r=localStorage.getItem(ACT_LS_KEY);return r?JSON.parse(r):[];}catch{return[];}
}
function _actSaveHistory(pts){
  try{localStorage.setItem(ACT_LS_KEY,JSON.stringify(pts.slice(-ACT_MAX_POINTS)));}catch{}
}
async function _actSnapshot(){
  try{
    // Forcer des données fraîches (bypass cache)
    Object.keys(_dmCache).forEach(k=>{ _dmCacheTime[k]=0; });
    const all=await dynmapGetAll();
    const data={};
    SRV.forEach(s=>{data[s]=(all[s]||[]).length;});
    const pts=_actLoadHistory();
    pts.push({ts:Date.now(),data});
    _actSaveHistory(pts);
    return pts;
  }catch{return _actLoadHistory();}
}
function _actFilterByHours(pts,hours){
  const cutoff=Date.now()-hours*3600*1000;
  return pts.filter(p=>p.ts>=cutoff);
}
function _actStartAutoSnapshot(){
  if(_actSnapshotTimer)return;
  _actSnapshotTimer=setInterval(async()=>{
    // Invalider le cache dynmap pour forcer des données fraîches
    Object.keys(_dmCache).forEach(k=>{ _dmCacheTime[k]=0; });
    await _actSnapshot();
    // Rafraîchir le graphe activité s'il est visible
    if($('s-activite')&&$('s-activite').classList.contains('active')){
      const pts=_actFilterByHours(_actLoadHistory(),actPeriod);
      if(pts.length){actData={points:pts};renderActivityChart(actData);renderActivityStats(actData);}
  ...
  },60*1000); // snapshot toutes les 1 minute
}

async function loadActivity(){
  const loadEl=$('act-loading');
  const emptyEl=$('act-empty');
  ...
  try{
    ...
    let pts=_actFilterByHours(_actLoadHistory(),actPeriod);
    if(pts.length){
      actData={points:pts};
      if(loadEl)loadEl.style.display='none';
      renderActivityChart(actData);
      renderActivityStats(actData);
      if($('act-last-update'))$('act-last-update').textContent='Mis à jour '+new Date().toLocaleTimeString('fr-FR');
    } else {
      if(loadEl)loadEl.style.display='none';
      if(emptyEl){emptyEl.style.display='flex';emptyEl.textContent='Collecte en cours — le graphique se remplit automatiquement toutes les minutes.';}
    }
  }catch(e){
    if(loadEl)loadEl.style.display='none';
    if(emptyEl){emptyEl.style.display='flex';emptyEl.textContent='Erreur : '+e.message;}
  }
}
