(function(){
  ...
  function watchGate() {
    ...
    // Le gate est absent du DOM ET pas de session = bypass
    // Si le gate est juste caché (display:none) c'est normal après login
  ...
  if(sessionStorage.getItem(SESSION_KEY)){
    ...
    unlockContent();
    // Fermer le loader automatiquement si déjà authentifié
    setTimeout(() => {
      const ldr = document.getElementById('ldr');
      if(ldr){ ldr.classList.add('out'); setTimeout(() => { ldr.style.display = 'none'; }, 800); }
    }, 1200);
  } else {
    lockContent();
    setTimeout(() => {
  ...
  window.pwCheck = async function(){
    ...
    const CORRECT = 'M0ss@dGl0ry#2024!Secure';
    if(val !== CORRECT){
      if(err){ err.textContent = 'CODE INVALIDE — ACCÈS REFUSÉ'; err.style.opacity = '1'; }
      if(inp){ inp.value = ''; inp.style.borderColor = 'rgba(255,24,64,.5)'; }
      setTimeout(() => {
        if(err) err.style.opacity = '0';
        if(inp) inp.style.borderColor = 'rgba(38,198,218,.2)';
      }, 3000);
      return;
    }

    // Mot de passe local OK → appel API pour obtenir le vrai token JWT
    ...
    try {
      ...
      if(d.ok){
        ...
        unlockContent();
        // Fermer le loader automatiquement
        setTimeout(() => {
          const ldr = document.getElementById('ldr');
          if(ldr){
            ...
            setTimeout(() => {
              ...
              if(typeof showToast === 'function') showToast('ACCÈS AUTORISÉ — BIENVENUE', 2500);
            }, 800);
          }
          if(typeof requestNotifPerms === 'function') requestNotifPerms();
        }, 600);
      } else {
        // Le serveur a refusé (token expiré, changement serveur, etc.)
        if(err){ err.textContent = d.error || 'ACCÈS REFUSÉ PAR LE SERVEUR'; err.style.opacity = '1'; }
        if(inp){ inp.value = ''; inp.style.borderColor = 'rgba(255,24,64,.5)'; }
        setTimeout(() => {
          if(err) err.style.opacity = '0';
          if(inp) inp.style.borderColor = 'rgba(38,198,218,.2)';
      ...
      // Serveur injoignable → fallback session locale (fonctionnalités API limitées)
      console.warn('[auth] Serveur injoignable, session locale uniquement');
      sessionStorage.setItem(SESSION_KEY, 'local_session');
      if(window._stopGateWatch) window._stopGateWatch();
      const gate = document.getElementById('pw-gate');
      ...
      setTimeout(() => {
        const ldr = document.getElementById('ldr');
        if(ldr){ ldr.classList.add('out'); setTimeout(() => { ldr.style.display = 'none'; if(typeof showToast === 'function') showToast('MODE HORS-LIGNE — API indisponible', 3000); }, 800); }
      }, 600);
    } finally {
