...
async function api(p,opts={}){
  // 1er essai avec token
  let r=await fetch(API+p,{...opts,headers:{..._authHeader(),'Content-Type':'application/json',...(opts.headers||{})}});
  // Si 401, retry sans token (endpoint public)
  if(r.status===401){r=await fetch(API+p,{...opts,headers:{'Content-Type':'application/json',...(opts.headers||{})}});}
  if(!r.ok)throw new Error('HTTP '+r.status);
  return r.json();
}
async function apiP(p,b){
  let r=await fetch(API+p,{method:'POST',headers:{'Content-Type':'application/json',..._authHeader()},body:JSON.stringify(b)});
  if(r.status===401){r=await fetch(API+p,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});}
  if(!r.ok)throw new Error('HTTP '+r.status);
  return r.json();
}
