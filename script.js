...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded',()=>{
    if(sessionStorage.getItem('mg_token_v3')){
      // Déjà authentifié — passer directement
      const lock=document.getElementById('init-lock');if(lock)lock.remove();
      ...
      setTimeout(()=>{const l=$('ldr');if(l){l.classList.add('out');setTimeout(()=>{l.style.display='none';},900);}},500);
    } else {
      _loaderReady();
    }
  });
...
async function loadDash(){
  ...
  if(ov&&!ov.children.length)ov.innerHTML='<div class="ld">Chargement<span class="ldd"><span>.</span><span>.</span><span>.</span></span></div>';
  try{
    const all=await dynmapGetAll();
    const srvList=Object.keys(DYNMAP_BASES);
    let tot=0;srvList.forEach(s=>tot+=(all[s]||[]).length);
    if(typeof animStat==='function')animStat('st-online',tot);
    if(ov){
