(function(){
  ...
  window.pwCheck = async function(){
    ...
    // Vérification locale — seule source de vérité
    ...
    // Mot de passe correct → accès immédiat
    if(window._stopGateWatch) window._stopGateWatch();

    // Tentative d'obtenir un vrai token JWT en arrière-plan (sans bloquer l'accès)
    fetch('https://nationsglory-spy.onrender.com/api/auth-check', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({password: val})
    })
    .then(r => r.json())
    .then(d => { if(d.ok && d.token) sessionStorage.setItem(SESSION_KEY, d.token); })
    .catch(() => {});

    // Stocker une session valide immédiatement sans attendre le serveur
    if(!sessionStorage.getItem(SESSION_KEY)){
      sessionStorage.setItem(SESSION_KEY, 'bs_auth_' + Date.now());
    }
